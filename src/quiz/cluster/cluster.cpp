...
void clusterHelper(int index, const std::vector<std::vector<float>>& points, 
    ...
    for (int id : tree->search(points[index], distanceTol)) {
...
int main ()
{
  ...
  /*  
  tree->sortVectors(points);
                      
    for (int i=0; i < tree->medianAlternativingVector.size(); i++) 
      	//exit(0);
    	//std::cout << tree->medianAlternativingVector.size() << std::endl;	
        tree->insert(tree->medianAlternativingVector[i],i); 
  */
